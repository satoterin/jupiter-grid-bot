...
export class ArbBot {
    ...
    //private wifMint: PublicKey = new PublicKey("EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm");
    private wifMint: PublicKey = new PublicKey("7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr");
    private usdcTokenAccount: PublicKey;
    private wifTokenAccount: PublicKey;
    private solBalance: number = 0;
    private usdcBalance: number = 0;
    private wifBalance: number = 0;
    ...
    constructor(config: ArbBotConfig) {
        ...
        this.wallet = Keypair.fromSecretKey(b58.decode(secretKey))
        this.usdcTokenAccount = getAssociatedTokenAddressSync(this.usdcMint, this.wallet.publicKey);
        this.wifTokenAccount = getAssociatedTokenAddressSync(this.wifMint, this.wallet.publicKey);
        if (targetGainPercentage) { this.targetGainPercentage = targetGainPercentage }
        if (checkInterval) { this.checkInterval = checkInterval }
        this.nextTrade = {
            inputMint: initialInputToken === SwapToken.SOL ? this.solMint.toBase58() : this.wifMint.toBase58(),
            outputMint: initialInputToken === SwapToken.SOL ? this.wifMint.toBase58() : this.solMint.toBase58(),
    ...
    private async evaluateQuoteAndSwap(quote: QuoteResponse): Promise<void> {
        console.log(`🔄 Evaluating quote: ${parseInt(quote.inAmount)/LAMPORTS_PER_SOL} ${quote.inputMint} -> ${parseInt(quote.outAmount)/LAMPORTS_PER_SOL} ${quote.outputMint}`);
        let difference = (parseInt(quote.outAmount) - this.nextTrade.nextTradeThreshold) / this.nextTrade.nextTradeThreshold;
        console.log(`📈 Current price: ${parseInt(quote.outAmount) / LAMPORTS_PER_SOL} is ${difference > 0 ? 'higher⬆️ ' : 'lower⬇️ '
            } than the next trade threshold: ${this.nextTrade.nextTradeThreshold / LAMPORTS_PER_SOL} by ${Math.abs(difference * 100).toFixed(2)}%.`);
    ...
    private async refreshBalances(): Promise<void> {
        try {
            const results = await Promise.allSettled([
                ...
                this.solanaConnection.getTokenAccountBalance(this.wifTokenAccount),
            ]);

            const solBalanceResult = results[0];
            const usdcBalanceResult = results[1];
            const wifBalanceResult = results[2];
            ...
            if (wifBalanceResult.status === 'fulfilled') {
                this.wifBalance = wifBalanceResult.value.value.uiAmount ?? 0;
            } else {
                this.wifBalance = 0;
            }
    ...
    private terminateSession(reason: string): void {
        ...
        console.log(`Current balances:\nSOL: ${this.solBalance / LAMPORTS_PER_SOL},\nUSDC: ${this.usdcBalance},\nPopcat: ${this.wifBalance / LAMPORTS_PER_SOL}`);
